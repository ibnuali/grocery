...
export const PlanService = {
  listPlans: (
    cursor?: string,
    limit = 20,
  ): Effect.Effect<ShoppingPlanPage, ApiError | NetworkError | DecodeError> => {
  ...
  deletePlan: (
    id: string,
  ): Effect.Effect<void, ApiError | NetworkError | DecodeError> =>
    request(`/api/v1/plans/${id}`, { method: "DELETE" }, Schema.Struct({})),
  ...
  checkItem: (
    ...
    { item_id: string; is_checked: boolean },
    ApiError | NetworkError | DecodeError
  > =>
    request(
