...
export const PlanRoute: React.FC<{ view: PlanRouteView }> = ({ view }) => {
  ...
  useEffect(() => {
    if (view !== "reconciliation" || !planId || activePlan?.id !== planId)
      return;
  ...
  const handleAddItem = async (item: {
    ...
    const prog = PlanService.addItem(
      ...
      Effect.catchAll(() => {
        toast(t("planDetail.errorAdd"), "error");
        return Effect.succeed(undefined);
      }),
    );
    await Effect.runPromise(prog);
    ...
    const prog = PlanService.addItem(
      ...
      Effect.map((addedItem) => {
        setActivePlan((current) =>
          current?.id === planId
            ? { ...current, items: [...(current.items ?? []), addedItem] }
            : current,
        );
        return addedItem;
      }),
      Effect.catchAll(() => {
        toast(t("inStore.errorAddUnplanned"), "error");
  ...
  const handleDeleteItem = async (itemId: string) => {
    ...
    const prog = PlanService.deleteItem(planId, itemId).pipe(
      Effect.map(() => loadPlanDetails(planId)),
      Effect.catchAll(() => {
        toast(t("planDetail.errorDelete"), "error");
        return Effect.succeed(undefined);
      }),
    );
    await Effect.runPromise(prog);
  ...
  if (view === "instore") {
    return (
      <InStoreView
        ...
        onComplete={async () => {
          const prog = PlanService.completePlan(activePlan.id).pipe(
            Effect.map((updatedPlan) => {
              setActivePlan((current) =>
                current?.id === updatedPlan.id
                  ? { ...updatedPlan, items: current.items }
                  : updatedPlan,
              );
              navigate(planDetailPath(updatedPlan.id));
            }),
            Effect.catchAll(() => {
              toast(t("inStore.errorComplete"), "error");
              return Effect.succeed(undefined);
            }),
          );
          await Effect.runPromise(prog);
        ...
        onBack={() => {
          void loadPlanDetails(activePlan.id);
          navigate(planDetailPath(activePlan.id));
        }}
        onProceedToReconcile={(items) => {
          setActivePlan((current) =>
            current?.id === activePlan.id
              ? { ...current, items: [...items] }
              : current,
          );
  ...
  return (
    <ReconciliationView
      ...
      onSuccess={(updated) => {
        ...
        navigate(planDetailPath(updated.id));
      }}
      onBack={() => {
        void loadPlanDetails(activePlan.id);
        navigate(planDetailPath(activePlan.id));
