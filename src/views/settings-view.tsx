...
const Feedback: React.FC<FeedbackProps> = ({ message, success = false }) => {
  ...
  return (
    <div
      className={`settings-feedback${success ? " settings-feedback--success" : ""}`}
      role={success ? "status" : "alert"}
    >
      {success ? (
        <CheckCircle2 aria-hidden="true" className="settings-feedback__icon" />
      ) : (
        <AlertCircle aria-hidden="true" className="settings-feedback__icon" />
      )}
      <span>{message}</span>
    </div>
  );
...
const SettingsRow: React.FC<SettingsRowProps> = ({
  ...
  const content = (
    <>
      ...
      {end ||
        (onClick ? (
          <ChevronRight aria-hidden="true" className="settings-row__chevron" />
        ) : null)}
    </>
  );
...
const SettingsSwitch: React.FC<{ active: boolean }> = ({ active }) => (
  <span
    className={`settings-switch${active ? " settings-switch--active" : ""}`}
    aria-hidden="true"
  >
    <span />
  </span>
...
export const SettingsView: React.FC<SettingsViewProps> = ({
  ...
  const accountErrorMessage = (error: unknown, fallback: string) => {
    if (error instanceof ApiError && error.code === "INVALID_PASSWORD")
      return t("settings.errorIncorrectPassword");
  ...
  const handleProfileSubmit = async (
    ...
    try {
      ...
      setProfileError(
        accountErrorMessage(error, t("settings.errorProfileSave")),
      );
    } finally {
  ...
  const handlePasswordSubmit = async (
    ...
    try {
      ...
      setPasswordError(
        accountErrorMessage(error, t("settings.errorPasswordSave")),
      );
    } finally {
  ...
  return (
    <div
      className={`settings-page${screen === "profile" ? " settings-page--profile" : ""}${resolvedTheme === "light" ? " settings-page--light" : ""}`}
    >
      <PageHeader
      ...
      {screen === "menu" ? (
        <>
          ...
          >
            <span className="settings-profile-row__avatar" aria-hidden="true">
              <UserRound />
            </span>
          ...
          >
            <SettingsRow
              icon={BellOff}
              label={t("settings.pauseNotifications")}
            />
            <SettingsRow
              icon={Settings2}
          ...
          >
            ...
            >
              ...
              <div className="settings-profile-field">
                ...
                <span className="settings-profile-field__value">
                  {user.email}
                </span>
              </div>
              <Button
